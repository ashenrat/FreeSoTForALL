...
HRESULT Cheat::Renderer::PresentHook(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags)
{
            ...
            for (auto l = 0u; l < levels.Count; l++)
            {
                ...
                for (auto a = 0u; a < actors.Count; a++)
                {
                    ...
                    if (cfg.aim.bEnable)
                    {
                        if (attachObject && attachObject->isCannon())
                        {
                            if (cfg.aim.cannon.b_chain_shots && actor->isShip())
                            {
                                do
                                {
                                    ...
                                    if (location.DistTo(cameraLoc) > 55000)
                                    {
                                        break;
                                    }
                                    int amount = 0;
                        ...
                        else if (cfg.aim.harpoon.bEnable && attachObject && attachObject->isHarpoon())
                        {
                            if (actor->isItem())
                            {
                                do
    ...
    if (bIsOpen) {
        ...
        ImGui::Begin("FreeSoTForAll", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
        ...
        if (ImGui::BeginTabBar("Bars")) {
            ...
            if (ImGui::BeginTabItem(ICON_FA_SLIDERS_H " Misc")) {
                ...
                if (ImGui::BeginChild("SwordMods", ImVec2(0.f, 200.f), true, 0 | ImGuiWindowFlags_NoScrollWithMouse))
                {
                    ...
                    ImGui::Checkbox("Clamping", &cfg.misc.sword.noclamp);
                    ImGui::Checkbox("No Slowness", &cfg.misc.sword.noblockreduce);
                }
                ImGui::EndChild();
